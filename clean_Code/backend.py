...
class SQLiteFTS5BM25:
    ...
    Reuses a single connection per instance to avoid open/close overhead.
    """
    def __init__(self, collection_name: str):
        self.db_path = os.path.join(CHROMA_DB_DIR, f"{collection_name}_fts5.db")
        self._conn: sqlite3.Connection | None = None
        # Single lock guarding every execute() / commit() on this connection.
        # Python's sqlite3 Connection is NOT thread-safe when opened with
        # check_same_thread=False: concurrent cursors on one connection can
        # corrupt cursor state (InterfaceError, SystemError, mangled rows).
        # WAL mode only helps when readers and writers use DIFFERENT
        # connections. We share one connection for pool reuse, so all
        # access must be serialized by this lock.
        self._lock = _FTS5_GLOBAL_LOCK
        self._init_db()

    def _get_conn(self) -> sqlite3.Connection:
        """Return a reusable connection (lazy-opened, kept alive).

        Uses WAL journal mode. All use of the returned connection MUST be
        guarded by ``self._lock`` — see __init__ for why.
        """
    ...
    def _init_db(self):
        with self._lock:
            ...
            # Migrate: if old 3-column table exists, rebuild with 4 columns
            try:
        ...
        with self._lock:
            ...
            try:
                for doc in documents:
                    ...
                    conn.execute("INSERT INTO hashes(content_hash) VALUES (?)", (h,))
                conn.commit()
            except Exception:
                conn.rollback()
                raise
    ...
    def close(self):
        """Explicitly close the SQLite connection to prevent handle leaks."""
        with self._lock:
    ...
    def delete_by_hashes(self, hashes: set[str]):
        ...
        with self._lock:
            conn = self._get_conn()
            # Identify rowids whose metadata contains a stale hash.
            # Fix OOM: iterate over cursor instead of fetchall()
            cursor = conn.execute("SELECT rowid, metadata_json FROM docs_fts")
            stale_rowids = []
            for rowid, meta_raw in cursor:
                try:
                    meta = json.loads(meta_raw)
                except Exception:
                    continue
                if meta.get("content_hash") in hashes:
