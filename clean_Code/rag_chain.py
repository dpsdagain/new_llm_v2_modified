...
_background_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="sentinel")
_rewrite_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="rewrite")
...
def build_rag_chain(db: Chroma, model: str | None = None):
    ...
    def _full_context_cache_chain(inputs: dict):
        ...
        while final_docs and sum(len(d.page_content) + _FMT_OVERHEAD_PER_CHUNK for d in final_docs) // 3 > _rag_budget:
        ...
        if should_summarize and not inputs.get("sentinel_future_active"):
            ...
            background_future = _background_executor.submit(_background_summarize, full_history)
